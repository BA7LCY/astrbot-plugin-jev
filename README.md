...
- 复用宿主的人格解析顺序：会话强制覆盖、当前对话人格、配置默认人格；显式无人格时为空，不擅自套用其他人格。
- 人格只传入其 `prompt`，不混入工具说明、人格 few-shot、技能或其他宿主动态 system prompt。人格文本最多 6000 字符。
- 变量仅做一次文字替换，不执行 Python/Jinja 等模板代码；每个变量在每个模板最多一次。
- 预览不调用模型，可选择近期会话的人格快照。刚开启人格引用尚无新消息时会显示空人格；下一条真实消息重新解析当前人格。
