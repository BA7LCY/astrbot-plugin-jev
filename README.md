...
- `{{persona}}`：当前会话实际生效的 AstrBot 人格提示词，也是模板中「机器人设定」这一行的唯一来源。**模板里写了这个变量就注入，没写就完全不向宿主解析、不发送**，没有额外开关；默认只有接话模板带 `{{persona}}`。
- 默认放行阈值 `threshold = 0.5`。线上实测概率天然分两簇：被 @ 或私聊追问 0.72～0.90，群聊无关闲聊与图片消息 0.06～0.45，发送复核 0.5～0.65；旧默认 0.65 会让复核阶段几乎全拦。
...
- 复核模板的取舍来自同一份真实 state 只改指令结构的重复调用：带完整人格 0.55/0.57，去掉人格 0.65/0.59，接话阶段两种写法差异在 ±0.05 内。
